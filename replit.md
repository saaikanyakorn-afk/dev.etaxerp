...
- **Sales Commission System:** `commission_rules` table (percentage/fixed, revenue-based, min target). `commission_records` table (employee × month/year, totalSales, commissionAmount, draft/approved status). Calculate endpoint aggregates approved tax invoices + invoices by salesperson, applies rule rate. Approve individually or batch. Payroll integration: approved commissions auto-included in `totalEarnings` during payroll calculation. Sidebar: HR → กฎคอมมิชชั่น, คำนวณค่าคอมมิชชั่น. All endpoints have tenant access checks.
...
**Database Servers page:** `client/src/pages/platform/database-servers.tsx` — UI only, hardcoded DEMO_MACHINES, no backend API yet.
