...
- **NO FALLBACK RULE (ABSOLUTE):** This is accounting software — NO hardcode fallback anywhere in the codebase. All accounting formulas, journal entries, and financial calculations MUST read from DB only. If data is not found in DB → error out explicitly, NEVER silently use hardcoded values. DEFAULT_FORMULAS is for seeding/reference only, NOT for runtime fallback. If any code is found using DEFAULT_FORMULAS as fallback, inform พี่ช้าง immediately.
- **DELETE COMMAND SAFETY RULE (ABSOLUTE — พี่ช้าง rule 2026-04-16):** DELETE on production (deep-main) is IRREVERSIBLE. No tool, no history, no person can bring deleted data back. This is REAL customer data. If a one-time migration includes DELETE:
  ...
  - **Never skip step 2-3.** Never rush. Never let anyone (พี่ทราย, พี่ช้าง, or yourself) pressure you into moving on before the DELETE code is removed.
  - Any non-DELETE one-time migration follows the same loop but DELETE requires extra caution because it is the only command where mistakes are truly permanent.
