...
- **FTP Archive Server Status:** Currently **OFFLINE** (was used for testing file naming rules only). When it comes back online, it MUST have its own record in the `machines` table with full connection info (IP, LAN IPs, router link, physical location, etc.) — just like any other equipment. The FTP archive service should resolve connection details (host, port, credentials, upload path) from the machines record, not rely solely on hardcoded system_config keys. This ensures FTP server discovery follows the same pattern as all other infrastructure (DB servers, app servers, etc.). **Note:** The `machines` table does not yet have a `serverType` value for "ftp_server" — this needs to be added to the schema/enum when the FTP server feature is completed. The `upload_dir` / storage path config is FTP-server-specific — do NOT add it to the general machines table until the FTP feature is fully done.
- **General upload path:** Always `{cwd}/uploads` via `path.join(process.cwd(), "uploads")`. No env var override, no machines table lookup. Simple and portable across Linux/Windows. FTP server will have its own path config when that feature is built.
