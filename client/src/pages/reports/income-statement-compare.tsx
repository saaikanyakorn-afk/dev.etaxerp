...
interface CompareData {
  revenues: CompareRow[];
  expenses: CompareRow[];
  totalRevenue: TotalRow;
  totalExpense: TotalRow;
  netIncome: TotalRow;
}
...
export default function IncomeStatementCompare() {
  ...
  const { data, isLoading, refetch } = useQuery<CompareData>({
    ...
    queryFn: async () => {
      const res = await fetch(
        `/api/reports/income-statement-compare?companyId=${companyId}&startDate1=${startDate1}&endDate1=${endDate1}&startDate2=${startDate2}&endDate2=${endDate2}`,
        { credentials: "include" }
      );
  ...
  const handleExcel = () => {
    ...
    rows.push(["รหัสบัญชี", "ชื่อบัญชี", "งวดที่ 1", "งวดที่ 2", "ผลต่าง", "เปลี่ยนแปลง %"]);
    rows.push(["", "รายได้", "", "", "", ""]);
    data.revenues.forEach((r) => rows.push([r.code, acctName(r), r.period1, r.period2, r.change, r.changePct]));
    rows.push(["", "รวมรายได้", data.totalRevenue.period1, data.totalRevenue.period2, data.totalRevenue.change, data.totalRevenue.changePct]);
    rows.push([]);
    rows.push(["", "ค่าใช้จ่าย", "", "", "", ""]);
    data.expenses.forEach((r) => rows.push([r.code, acctName(r), r.period1, r.period2, r.change, r.changePct]));
    rows.push(["", "รวมค่าใช้จ่าย", data.totalExpense.period1, data.totalExpense.period2, data.totalExpense.change, data.totalExpense.changePct]);
    rows.push([]);
    rows.push(["", "กำไร(ขาดทุน)สุทธิ", data.netIncome.period1, data.netIncome.period2, data.netIncome.change, data.netIncome.changePct]);
    const ws = XLSX.utils.aoa_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "IncomeStatementCompare");
    XLSX.writeFile(wb, "income-statement-compare.xlsx");
  };
  ...
  return (
    <ReportLayout fullWidth title="งบกำไร/ขาดทุนเปรียบเทียบ" icon={<TrendingUp className="h-5 w-5" />}>
            ...
            <CardContent className="p-0">
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {renderTotalRow("รวมรายได้", data.totalRevenue)}
                  ...
                  {renderTotalRow("รวมค่าใช้จ่าย", data.totalExpense)}
                  {renderTotalRow("กำไร(ขาดทุน)สุทธิ", data.netIncome, true)}
                </TableBody>
              </Table>
            </CardContent>
          </Card>
