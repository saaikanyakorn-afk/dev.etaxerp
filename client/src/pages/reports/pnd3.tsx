...
export default function Pnd3Page() {
  ...
  const { data, isLoading } = useQuery<any[]>({
    queryKey: ["/api/reports/pnd3", companyId, startDate, endDate],
    queryFn: async () => {
      const p = new URLSearchParams({ companyId });
      if (startDate) p.set("startDate", startDate);
      if (endDate) p.set("endDate", endDate);
      const r = await fetch(`/api/reports/wht-summary?${p}&pndType=3`, { credentials: "include" });
      if (!r.ok) return [];
      return r.json();
    },
    enabled: !!companyId,
  ...
  return (
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2" style={{ color: "#03c9d7" }}>
            <FileText className="h-5 w-5" /> T5: รายงาน ภงด 3
          </h1>
          <p className="text-sm text-gray-500">รายงานสรุปภาษีหัก ณ ที่จ่าย ภงด.3 (บุคคลธรรมดา)</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="py-12 text-center text-gray-400">ไม่พบรายการ ภงด.3 ในช่วงเวลาที่เลือก</CardContent></Card>
      ) : (
        <Card>
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-sm" data-testid="pnd3-table">
                <thead>
                ...
                <tbody>
                  {data.map((r: any, i: number) => (
                    <tr key={i} className="border-b hover:bg-gray-50" data-testid={`pnd3-row-${i}`}>
