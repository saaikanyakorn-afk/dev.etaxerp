...
import { FolderKanban, FileText, Calendar as CalendarIcon, ChevronDown, ChevronRight, BarChart3, Printer } from "lucide-react";
...
export default function SalesByProject() {
  ...
  const { data, isLoading } = useQuery<any>({
    queryKey: ["/api/reports/sales-summary", companyId, docType, dateFrom, dateTo, "project"],
    queryFn: async () => {
      if (!companyId) return null;
      const params = new URLSearchParams({ companyId: String(companyId), docType, dateFrom, dateTo, groupBy: "project" });
  ...
  const exportData = useMemo(() => {
    ...
    groups.forEach((g: any) => {
      (g.items || []).forEach((item: any) => {
        rows.push({ project: g.name, docNo: item.docNo, docDate: item.docDate, customerName: item.customerName, subtotal: parseFloat(item.subtotal || "0"), vatAmount: parseFloat(item.vatAmount || "0"), totalAmount: parseFloat(item.totalAmount || "0") });
      });
    });
    return rows;
  }, [groups]);
  ...
  return (
    <ReportLayout title="R8: ยอดขาย - ตามโครงการ/เอกสาร" icon={<FolderKanban className="h-5 w-5" />}>
      <div className="flex justify-end mb-2 gap-2">
        ...
        <ListExportButton data={exportData} columns={exportColumns} fileName="R8-ยอดขาย-ตามโครงการ" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          { label: "จำนวนโครงการ", value: groups.length.toLocaleString("th-TH"), color: "#05b187", icon: FolderKanban },
        ].map((stat, i) => (
            ...
            <Table>
              <TableHeader className="bg-slate-100">
                <TableRow className="hover:bg-transparent h-11">
                  ...
                  <TableHead className="text-sm font-medium text-slate-700">โครงการ</TableHead>
