                ...
                <tbody>
                  {items.map((item, idx) => (
                    <tr key={idx} className="align-top hover:bg-sky-50/30 border-b border-slate-300">
                      ...
                      <td className="px-1 pt-1.5">
                        <textarea className="w-full min-w-0 text-xs border border-dashed rounded px-2 py-1.5 resize-y min-h-[28px] focus:outline-none focus:ring-1 focus:ring-sky-400 bg-transparent" rows={1} placeholder="พิมพ์ชื่อสินค้า/บริการ" value={item.productName} onChange={e => updateItem(idx, "productName", e.target.value)} data-testid={`input-product-name-${idx}`} />
                      </td>
                      <td className="px-1 pt-1.5">
