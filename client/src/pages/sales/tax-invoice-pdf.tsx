...
import { ArrowLeft, FileText, Receipt, FlaskConical, Loader2, CheckCircle2, AlertCircle, Activity, WifiOff, RefreshCw } from "lucide-react";
...
function AbbreviatedTaxInvoice({ data, company }: { data: any; company: any }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: "11px", marginBottom: "6px" }}>
        {items.length > 1 && <div style={{ display: "flex", justifyContent: "space-between" }}><span>รวม ({items.length} รายการ)</span><span>{formatMoney(totalAmount)}</span></div>}
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}><span>ราคาสินค้า (ก่อน VAT)</span><span>{formatMoney(totalAmount - vatAmount)}</span></div>
    ...
    <Layout>
      <div className="min-h-[60vh] flex items-center justify-center" data-testid="server-error-screen">
        <div className="text-center max-w-lg mx-auto px-6 py-12">
          ...
          <p className="text-gray-500 text-sm mb-8">อาจเกิดจากการขาดการเชื่อมต่ออินเทอร์เน็ต หรือเซิร์ฟเวอร์กำลังรีสตาร์ท</p>
...
export default function TaxInvoicePdf() {
  ...
  const pdfUrl = isThermal ? "" : `/api/documents/tax_invoice/${id}/pdf${printType !== "tax_invoice" ? `?printType=${printType}` : ""}`;
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {demoResult.stats && (
                    <span className="text-[10px] text-gray-400 flex items-center gap-0.5">
                      <Activity className="h-3 w-3" />{demoResult.stats.pdfSizeKB}KB | RAM {demoResult.stats.memoryBeforeMB}→{demoResult.stats.memoryAfterMB}MB
                    </span>
                  )}
                </div>
              )}
