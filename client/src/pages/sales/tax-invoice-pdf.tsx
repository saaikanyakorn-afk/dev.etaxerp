...
import { ArrowLeft, FileText, Receipt, Loader2, WifiOff, RefreshCw } from "lucide-react";
        ...
        <div className="text-center max-w-lg mx-auto px-6 py-12">
          ...
          <div className="space-y-3">
            <Button onClick={handleRetry} disabled={retrying} className="w-full bg-[#fb9678] hover:bg-[#e8856a] text-white h-12 text-base" data-testid="btn-retry-connection">
              {retrying ? (
                <><Loader2 className="h-5 w-5 animate-spin mr-2" />กำลังเชื่อมต่อใหม่... {countdown > 0 && `(${countdown})`}</>
              ) : (
                <><RefreshCw className="h-5 w-5 mr-2" />ลองเชื่อมต่อใหม่</>
              )}
            </Button>
...
export default function TaxInvoicePdf() {
  ...
  const pdfUrl = `/api/documents/tax_invoice/${id}/pdf?view=1&printType=${printType}`;
        ...
        {isThermal ? (
          ...
          <div className="max-w-3xl mx-auto rounded overflow-hidden border border-slate-200">
            <iframe
              ...
              title={data.taxInvoiceNo || "Tax Invoice PDF"}
            />
          </div>
        )}
      </div>
