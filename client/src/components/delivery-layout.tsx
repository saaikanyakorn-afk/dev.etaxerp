...
const DELIVERY_NAV: NavItem[] = [
  { label: "ภาพรวม", icon: LayoutDashboard, href: "/delivery/dashboard" },
  { label: "จัดส่งสินค้า (Pick-Pack-Ship)", icon: Package, href: "/delivery/fulfillment" },
  { label: "ใบปะหน้าพัสดุ", icon: Tag, href: "/delivery/shipping-labels" },
  { label: "ติดตามพัสดุ", icon: MapPin, href: "/delivery/tracking" },
  { label: "แจ้ง Tracking ผ่าน LINE", icon: MessageCircle, href: "/delivery/line-notify" },
  { label: "รายการจัดส่ง", icon: ClipboardList, href: "/delivery/shipments" },
  { label: "สแกนพัสดุ", icon: ScanLine, href: "/delivery/scan" },
  { label: "ตั้งค่า", icon: Settings, href: "/delivery/settings" },
];
    ...
    <div className="min-h-screen bg-gray-50/50 flex font-sans">
      ...
      <aside className={cn(
        ...
        <div className="flex items-center px-4 border-b border-sidebar-border shrink-0 relative overflow-hidden" style={{ background: "#03c9d7", height: "68px" }}>
          <Link href="/delivery/dashboard" className="flex items-center group cursor-pointer flex-1 justify-center relative z-10 gap-2">
            <Truck className="h-6 w-6 text-white" />
            <span className="text-white font-bold text-lg" data-testid="text-delivery-title">Delivery Hub</span>
          </Link>
          <button
            ...
            return (
              <Link key={item.href} href={item.href}>
                <span
                  ...
                  data-testid={`menu-delivery-${item.href.split("/").pop()}`}
                >
        ...
        <div className="p-4 border-t border-sidebar-border shrink-0">
          <div className="flex items-center gap-3 px-2 py-2">
            <div className="h-10 w-10 rounded-full flex items-center justify-center shrink-0" style={{ background: "#03c9d7" }}>
      ...
      <main className="flex-1 md:ml-64 flex flex-col min-h-screen print:!ml-0">
        <header className="h-16 bg-white border-b flex items-center justify-between px-4 md:px-6 sticky top-0 z-10 print:!hidden shadow-sm">
          <div className="flex items-center gap-2 md:gap-4 flex-1">
            ...
            <span className="text-lg font-bold" style={{ color: "#03c9d7" }} data-testid="text-header-title">Delivery Hub</span>
          </div>
          <div className="flex items-center gap-1">
            <SubscriptionNavButton />
            <button
              className="h-10 w-10 rounded-full flex items-center justify-center relative transition-colors hover:bg-[#e6fafb]"
              style={{ color: "#fec90f" }}
