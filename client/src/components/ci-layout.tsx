...
const CI_NAV: NavItem[] = [
  { label: "ภาพรวมผู้บริหาร", icon: LayoutDashboard, href: "/ci/executive" },
  { label: "วิเคราะห์ช่องทาง", icon: BarChart3, href: "/ci/channel" },
  { label: "สินค้า & กำไร", icon: Package, href: "/ci/product" },
  { label: "แคมเปญโฆษณา", icon: Megaphone, href: "/ci/campaign" },
  { label: "ไลฟ์คอมเมิร์ซ", icon: Radio, href: "/ci/live" },
  { label: "แจ้งเตือนอัจฉริยะ", icon: AlertTriangle, href: "/ci/alerts" },
];

const CI_PRIMARY = "#7c3aed";
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50/50 flex font-sans">
      ...
      <aside className={cn(
        ...
        <div className="flex items-center px-4 border-b border-sidebar-border shrink-0 relative overflow-hidden" style={{ background: CI_PRIMARY, height: "68px" }}>
          <Link href="/ci/executive" className="flex items-center group cursor-pointer flex-1 justify-center relative z-10 gap-2">
            <BrainCircuit className="h-6 w-6 text-white" />
            <span className="text-white font-bold text-lg" data-testid="text-ci-title">Commerce Intelligence</span>
          </Link>
          <button
            className="md:hidden ml-2 p-1 rounded hover:bg-white/20 text-white relative z-10"
            onClick={() => setMobileMenuOpen(false)}
            data-testid="button-ci-close-mobile"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-4 shrink-0">
          {!isAccountingFirm ? (
            <div
              ...
              data-testid="ci-text-company-name"
            >
            ...
            <Popover open={companyPopoverOpen} onOpenChange={(open) => { setCompanyPopoverOpen(open); if (!open) setCompanySearch(""); }}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  data-testid="ci-button-company-switcher"
                >
              ...
              <PopoverContent className="w-64 p-0 bg-sidebar border-sidebar-border" align="start">
                <div className="p-2 border-b border-sidebar-border">
                  <div className="relative">
                    ...
                    <Input
                      data-testid="ci-input-company-search"
                  ...
                  {filteredCompanies.length === 0 ? (
                    ...
                    filteredCompanies.map((company: any) => (
                      <div
                        ...
                        data-testid={`ci-menu-company-${company.id}`}
                      >
                        <button
                          ...
                          data-testid={`ci-button-select-company-${company.id}`}
                        >
                          {company.name}
                        </button>
                        <button
                          ...
                          data-testid={`ci-button-set-primary-${company.id}`}
                        >
                ...
                <div className="border-t border-sidebar-border px-3 py-1.5 flex items-center gap-1.5">
                  <span className="inline-flex items-center justify-center h-4 min-w-[16px] px-1 rounded-full text-[9px] font-bold text-white" style={{ background: CI_PRIMARY }}>{companies.length}</span>
        ...
        <nav ref={sidebarNavRef} className="flex-1 overflow-y-auto px-2 space-y-0.5 pb-4 min-h-0">
          {CI_NAV.map((item) => {
            const isActive = location === item.href || (item.href !== "/ci/executive" && location.startsWith(item.href + "/"));
            return (
              <Link key={item.href} href={item.href}>
                <span
                  ...
                  style={isActive ? { background: CI_PRIMARY } : undefined}
                  data-sidebar-active={isActive ? "true" : undefined}
                  data-testid={`ci-menu-${item.href.split("/").pop()}`}
                >
        ...
        <div className="px-3 pb-3 shrink-0">
          <Link href="/">
            <span className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all cursor-pointer border-2 border-[#fb9678] text-[#fb9678] hover:bg-[#fb9678] hover:text-white" data-testid="ci-link-back-home">
        ...
        <div className="p-4 border-t border-sidebar-border shrink-0">
          <div className="flex items-center gap-3 px-2 py-2">
            <div className="h-10 w-10 rounded-full flex items-center justify-center shrink-0" style={{ background: CI_PRIMARY }}>
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-semibold truncate" data-testid="ci-text-user-name">{user.fullName}</p>
        ...
        <header className="h-16 bg-white border-b flex items-center justify-between px-4 md:px-6 sticky top-0 z-10 print:!hidden shadow-sm">
          <div className="flex items-center gap-2 md:gap-4 flex-1">
            <button
              ...
              data-testid="ci-button-open-mobile-menu"
            >
              <Menu className="h-5 w-5" />
            </button>
            <span className="text-lg font-bold" style={{ color: CI_PRIMARY }} data-testid="ci-text-header-title">Commerce Intelligence</span>
          </div>
          <div className="flex items-center gap-1">
            <SubscriptionNavButton />
            <button
              className="h-10 w-10 rounded-full flex items-center justify-center relative transition-colors hover:bg-purple-50"
              style={{ color: "#fec90f" }}
              data-testid="ci-button-notifications"
            >
    ...
    {switchTarget && (
      <div className="fixed inset-0 z-[10000] bg-black/50 flex items-center justify-center" data-testid="ci-switch-company-dialog">
        <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm mx-4">
          ...
          <div className="flex gap-3">
            <button onClick={() => setSwitchTarget(null)} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors" data-testid="ci-btn-switch-cancel">ยกเลิก</button>
            <button onClick={() => { const targetId = switchTarget.id; setSwitchTarget(null); setSelectedCompanyId(targetId); queryClient.invalidateQueries({ queryKey: ["/api/permissions/me"] }); queryClient.invalidateQueries({ queryKey: ["/api/notifications"] }); setLocation("/ci/executive"); }} className="flex-1 px-4 py-2.5 text-white rounded-lg text-sm font-semibold transition-colors" style={{ background: CI_PRIMARY }} data-testid="ci-btn-switch-confirm">ยืนยันเปลี่ยน</button>
          </div>
        </div>
      </div>
    )}
