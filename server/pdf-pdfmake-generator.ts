...
function buildDocDefinition(opts: GeneratePdfOptions): TDocumentDefinitions {
  ...
  if (settings.showSignature !== false) {
    ...
    if (sigImg) {
      try {
        sigRight.push({ image: sigImg, fit: [130, SIG_IMG_H], alignment: "center", margin: [0, 0, 0, 2] });
      } catch {}
    } else {
      sigRight.push({ canvas: [{ type: "rect", x: 0, y: 0, w: 1, h: SIG_IMG_H + 2, color: "white" }] });
    }
    ...
    const sigLeft: Content[] = [
      { canvas: [{ type: "rect", x: 0, y: 0, w: 1, h: SIG_IMG_H + 2, color: "white" }] },
