...
export async function runOneTimeSchemaV85Migration() {
  try {
    ...
    if ((flagRows.rows || []).length > 0) {
      await runOneTimeSchemaV86Migration();
      return;
    }
...
// ─── Schema v86 — warehouse_id on invoice_items + purchase_invoice_items ──────
...
export async function runOneTimeSchemaV86Migration() {
  try {
    ...
    await db.execute(sql`
      INSERT INTO system_config (config_key, config_value, description)
      VALUES (${MIGRATION_KEY_V86}, ${"done_" + new Date().toISOString()}, 'Add warehouse_id to invoice_items and purchase_invoice_items')
      ON CONFLICT (config_key) DO NOTHING
    `);

    console.log("[OneTimeMigration] ✅ Schema v86 migration complete");
  } catch (err: any) {
    console.error("[OneTimeMigration] ❌ Error v86:", err.message);
  }
}
