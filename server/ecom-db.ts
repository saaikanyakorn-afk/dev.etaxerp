...
const isProduction = process.env.NODE_ENV === "production";
const ecomConfig = getEcomDbUrl();
console.log(`[EcomDB] Connection: ${ecomConfig.label}`);

const ecomPool = new pg.Pool({
  connectionString: ecomConfig.url,
...
if (isProduction) {
  ...
  setInterval(async () => {
    try {
      const client = await ecomPool.connect();
