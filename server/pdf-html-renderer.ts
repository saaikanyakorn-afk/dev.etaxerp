...
export function renderDocumentHtml(opts: GeneratePdfOptions): string {
  ...
  const docTitleHtml = `
    <div style="display:flex;flex-direction:column;align-items:flex-end">
      <div style="padding:4px 12px;border-radius:6px;-webkit-border-radius:6px;background:${theme.bg};text-align:right">
  ...
  const bankSideBoxHtml = hasBankInfo ? `
    <div style="width:192px;border:1px solid ${theme.light || "#e5e7eb"};border-radius:4px;-webkit-border-radius:4px;padding:10px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:${theme.bg || "#fafafa"}">
      ...
      <div style="font-size:9px;color:#4b5563;text-align:center;width:100%">
        ...
        ${(parseFloat(String(doc.subtotal || "0")) + parseFloat(String(doc.vatAmount || "0")) - withholdingTax) > 0 ? `<div style="font-weight:600;font-size:10px;margin-top:4px;color:${primary}">จำนวนเงิน: ${fmt(parseFloat(String(doc.subtotal || "0")) + parseFloat(String(doc.vatAmount || "0")) - withholdingTax)} บาท</div>` : ""}
      </div>
    </div>
  ` : "";
