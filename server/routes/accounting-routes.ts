...
app.get("/api/accounting-formulas/available", requireAuth, async (req, res) => {
  try {
    ...
    if (documentType) {
      ...
      const allBizTypes = [formulaBusinessType, ...["trading", "service", "ecommerce", "ecommerce_commission", "platform_fee", "online_shop", "online_shop_commission", "mixed"].filter(bt => bt !== formulaBusinessType)];
