...
export function registerRestaurantRoutes(app: Express) {
  ...
  app.get("/api/restaurant/areas", requireAuth, async (req, res) => {
    try {
      ...
      const rows = await db.select().from(restaurantAreas).where(eq(restaurantAreas.companyId, companyId)).orderBy(restaurantAreas.sortOrder);
      res.json(rows);
  ...
  app.post("/api/restaurant/areas", requireAuth, async (req, res) => {
    try {
      const [created] = await db.insert(restaurantAreas).values(req.body).returning();
      res.json(created);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.put("/api/restaurant/areas/:id", requireAuth, async (req, res) => {
    try {
      const [updated] = await db.update(restaurantAreas).set(req.body).where(eq(restaurantAreas.id, Number(req.params.id))).returning();
      res.json(updated);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.delete("/api/restaurant/areas/:id", requireAuth, async (req, res) => {
    try {
      await db.delete(restaurantAreas).where(eq(restaurantAreas.id, Number(req.params.id)));
  ...
  app.get("/api/restaurant/tables", requireAuth, async (req, res) => {
    try {
      const companyId = Number(req.query.companyId);
      if (!companyId) return res.status(400).json({ message: "companyId required" });
      const rows = await db.select().from(restaurantTables).where(eq(restaurantTables.companyId, companyId)).orderBy(restaurantTables.sortOrder);
      res.json(rows);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.post("/api/restaurant/tables", requireAuth, async (req, res) => {
    try {
      const [created] = await db.insert(restaurantTables).values(req.body).returning();
      res.json(created);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.put("/api/restaurant/tables/:id", requireAuth, async (req, res) => {
    try {
      const [updated] = await db.update(restaurantTables).set(req.body).where(eq(restaurantTables.id, Number(req.params.id))).returning();
      res.json(updated);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.delete("/api/restaurant/tables/:id", requireAuth, async (req, res) => {
    try {
      await db.delete(restaurantTables).where(eq(restaurantTables.id, Number(req.params.id)));
      res.json({ success: true });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.get("/api/restaurant/menu-categories", requireAuth, async (req, res) => {
    try {
      const companyId = Number(req.query.companyId);
      if (!companyId) return res.status(400).json({ message: "companyId required" });
      const rows = await db.select().from(menuCategories).where(eq(menuCategories.companyId, companyId)).orderBy(menuCategories.sortOrder);
      res.json(rows);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.post("/api/restaurant/menu-categories", requireAuth, async (req, res) => {
    try {
      const [created] = await db.insert(menuCategories).values(req.body).returning();
      res.json(created);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.put("/api/restaurant/menu-categories/:id", requireAuth, async (req, res) => {
    try {
      const [updated] = await db.update(menuCategories).set(req.body).where(eq(menuCategories.id, Number(req.params.id))).returning();
      res.json(updated);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.delete("/api/restaurant/menu-categories/:id", requireAuth, async (req, res) => {
    try {
      await db.delete(menuCategories).where(eq(menuCategories.id, Number(req.params.id)));
      res.json({ success: true });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.get("/api/restaurant/menu-items", requireAuth, async (req, res) => {
    try {
      const companyId = Number(req.query.companyId);
      if (!companyId) return res.status(400).json({ message: "companyId required" });
      const rows = await db.select().from(menuItems).where(eq(menuItems.companyId, companyId)).orderBy(menuItems.sortOrder);
      res.json(rows);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.post("/api/restaurant/menu-items", requireAuth, async (req, res) => {
    try {
      const [created] = await db.insert(menuItems).values(req.body).returning();
      res.json(created);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.put("/api/restaurant/menu-items/:id", requireAuth, async (req, res) => {
    try {
      const [updated] = await db.update(menuItems).set(req.body).where(eq(menuItems.id, Number(req.params.id))).returning();
      res.json(updated);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.get("/api/restaurant/modifier-groups", requireAuth, async (req, res) => {
    try {
      const companyId = Number(req.query.companyId);
      if (!companyId) return res.status(400).json({ message: "companyId required" });
      const groups = await db.select().from(menuModifierGroups).where(eq(menuModifierGroups.companyId, companyId));
      const groupIds = groups.map(g => g.id);
      let options: any[] = [];
      if (groupIds.length > 0) {
        options = await db.select().from(menuModifierOptions)
  ...
  app.post("/api/restaurant/modifier-groups", requireAuth, async (req, res) => {
    try {
      ...
      const [group] = await db.insert(menuModifierGroups).values(groupData).returning();
      if (options?.length) {
        for (const opt of options) {
          await db.insert(menuModifierOptions).values({ ...opt, groupId: group.id });
        }
      }
      res.json(group);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  ...
  app.put("/api/restaurant/modifier-groups/:id", requireAuth, async (req, res) => {
    try {
      const { options, ...groupData } = req.body;
      const [updated] = await db.update(menuModifierGroups).set(groupData).where(eq(menuModifierGroups.id, Number(req.params.id))).returning();
      if (options) {
        await db.delete(menuModifierOptions).where(eq(menuModifierOptions.groupId, updated.id));
        for (const opt of options) {
          await db.insert(menuModifierOptions).values({ ...opt, groupId: updated.id });
        }
      }
      res.json(updated);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  ...
  app.delete("/api/restaurant/modifier-groups/:id", requireAuth, async (req, res) => {
    try {
      ...
      await db.delete(menuModifierOptions).where(eq(menuModifierOptions.groupId, id));
      await db.delete(menuItemModifiers).where(eq(menuItemModifiers.modifierGroupId, id));
      await db.delete(menuModifierGroups).where(eq(menuModifierGroups.id, id));
      res.json({ success: true });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.get("/api/restaurant/menu-item-modifiers/:menuItemId", requireAuth, async (req, res) => {
    try {
      const rows = await db.select().from(menuItemModifiers).where(eq(menuItemModifiers.menuItemId, Number(req.params.menuItemId)));
      res.json(rows);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.post("/api/restaurant/menu-item-modifiers", requireAuth, async (req, res) => {
    try {
      ...
      for (const gid of (modifierGroupIds || [])) {
        await db.insert(menuItemModifiers).values({ menuItemId, modifierGroupId: gid });
      }
      res.json({ success: true });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.get("/api/restaurant/orders", requireAuth, async (req, res) => {
    try {
      ...
      if (!companyId) return res.status(400).json({ message: "companyId required" });
      let query = db.select().from(restaurantOrders).where(eq(restaurantOrders.companyId, companyId)).orderBy(desc(restaurantOrders.createdAt));
      const rows = status
        ? await db.select().from(restaurantOrders).where(and(eq(restaurantOrders.companyId, companyId), eq(restaurantOrders.status, status))).orderBy(desc(restaurantOrders.createdAt))
        : await db.select().from(restaurantOrders).where(eq(restaurantOrders.companyId, companyId)).orderBy(desc(restaurantOrders.createdAt));
      res.json(rows);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.get("/api/restaurant/orders/:id", requireAuth, async (req, res) => {
    try {
      const [order] = await db.select().from(restaurantOrders).where(eq(restaurantOrders.id, Number(req.params.id)));
  ...
  app.post("/api/restaurant/orders", requireAuth, async (req, res) => {
    try {
      ...
      if (orderData.tableId) {
        await db.update(restaurantTables).set({ status: "occupied" }).where(eq(restaurantTables.id, orderData.tableId));
      }
      res.json(order);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.post("/api/restaurant/orders/:id/items", requireAuth, async (req, res) => {
    try {
      ...
      const allItems = await db.select().from(restaurantOrderItems).where(eq(restaurantOrderItems.orderId, orderId));
      const subtotal = allItems.reduce((s, i) => s + parseFloat(i.unitPrice) * i.quantity, 0);
      await db.update(restaurantOrders).set({ subtotal: String(subtotal) }).where(eq(restaurantOrders.id, orderId));
      res.json({ success: true });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.put("/api/restaurant/orders/:id/status", requireAuth, async (req, res) => {
    try {
      ...
      if (status === "paid" || status === "cancelled") {
        if (updated.tableId) {
          await db.update(restaurantTables).set({ status: "available" }).where(eq(restaurantTables.id, updated.tableId));
        }
      }
      res.json(updated);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  ...
  app.post("/api/restaurant/orders/:id/calculate", requireAuth, async (req, res) => {
    try {
      ...
      const items = await db.select().from(restaurantOrderItems).where(and(eq(restaurantOrderItems.orderId, orderId), sql`${restaurantOrderItems.status} != 'cancelled'`));
  ...
  app.post("/api/restaurant/orders/:id/send-to-kitchen", requireAuth, async (req, res) => {
    try {
      ...
      if (!order) return res.status(404).json({ message: "Order not found" });
      const pendingItems = await db.select().from(restaurantOrderItems)
      ...
      for (const item of pendingItems) {
        await db.update(restaurantOrderItems).set({ status: "preparing", sentToKitchenAt: new Date() })
          .where(eq(restaurantOrderItems.id, item.id));
      }
      const tableName = order.tableId
        ? (await db.select({ name: restaurantTables.name }).from(restaurantTables).where(eq(restaurantTables.id, order.tableId)))[0]?.name
        : null;
      ...
      await db.update(restaurantOrders).set({ status: "preparing" }).where(eq(restaurantOrders.id, orderId));
  ...
  app.get("/api/restaurant/kitchen-tickets", requireAuth, async (req, res) => {
    try {
      const companyId = Number(req.query.companyId);
      if (!companyId) return res.status(400).json({ message: "companyId required" });
      const tickets = await db.select().from(kitchenTickets)
  ...
  app.put("/api/restaurant/kitchen-tickets/:id/status", requireAuth, async (req, res) => {
    try {
      const { status } = req.body;
      const [updated] = await db.update(kitchenTickets).set({
        status, ...(status === "done" ? { completedAt: new Date() } : {}),
      }).where(eq(kitchenTickets.id, Number(req.params.id))).returning();
      if (status === "done") {
        await db.update(restaurantOrderItems).set({ status: "ready" })
  ...
  app.put("/api/restaurant/order-items/:id/status", requireAuth, async (req, res) => {
    try {
      const { status } = req.body;
      const [updated] = await db.update(restaurantOrderItems).set({
  ...
  app.put("/api/restaurant/splits/:id/pay", requireAuth, async (req, res) => {
    try {
      ...
      if (allSplits.every(s => s.paid)) {
        await db.update(restaurantOrders).set({ status: "paid", paidAt: new Date() }).where(eq(restaurantOrders.id, updated.orderId));
        const [order] = await db.select().from(restaurantOrders).where(eq(restaurantOrders.id, updated.orderId));
        if (order?.tableId) {
  ...
  app.post("/api/restaurant/orders/:id/move-table", requireAuth, async (req, res) => {
    try {
      ...
      await db.update(restaurantOrders).set({ tableId: newTableId }).where(eq(restaurantOrders.id, orderId));
      await db.update(restaurantTables).set({ status: "occupied" }).where(eq(restaurantTables.id, newTableId));
      res.json({ success: true });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.post("/api/restaurant/orders/merge", requireAuth, async (req, res) => {
    try {
      ...
      const sourceItems = await db.select().from(restaurantOrderItems).where(eq(restaurantOrderItems.orderId, sourceOrderId));
      for (const item of sourceItems) {
        await db.update(restaurantOrderItems).set({ orderId: targetOrderId }).where(eq(restaurantOrderItems.id, item.id));
      }
      const [sourceOrder] = await db.select().from(restaurantOrders).where(eq(restaurantOrders.id, sourceOrderId));
      if (sourceOrder?.tableId) {
        await db.update(restaurantTables).set({ status: "available" }).where(eq(restaurantTables.id, sourceOrder.tableId));
      }
      await db.update(restaurantOrders).set({ status: "cancelled" }).where(eq(restaurantOrders.id, sourceOrderId));
      res.json({ success: true });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
