...
import { eq, and, desc, sql, count, sum, not, inArray, like } from "drizzle-orm";
...
export function registerFixedAssetsRoutes(app: Express) {
  ...
  app.get("/api/fixed-assets/depreciation-journals", requireAuth, async (req, res) => {
    try {
      const companyId = Number(req.query.companyId);
      if (!companyId) return res.status(400).json({ message: "กรุณาระบุ companyId" });
      const entriesRaw = await db.execute(sql`
        SELECT id, company_id AS "companyId", entry_no AS "entryNo",
               entry_date AS "entryDate", reference, description, status,
               journal_book AS "journalBook", source_doc_id AS "sourceDocId",
               source_doc_type AS "sourceDocType", is_auto_generated AS "isAutoGenerated",
      ...
      `);
      const entries = entriesRaw.rows as any[];
      const jeIds = entries.map((e: any) => e.id);
      let allLines: any[] = [];
      if (jeIds.length > 0) {
        allLines = (await db.execute(sql`
          SELECT id, journal_entry_id AS "journalEntryId",
                 account_code AS "accountCode", account_name AS "accountName",
                 debit, credit, description
          FROM journal_lines
          WHERE journal_entry_id = ANY(${jeIds})
        `)).rows as any[];
      }
      const linesByJeId: Record<number, any[]> = {};
      allLines.forEach((l: any) => { (linesByJeId[l.journalEntryId] ??= []).push(l); });
      const result = entries.map((je: any) => ({ ...je, lines: linesByJeId[je.id] || [] }));
      res.json(result);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
      ...
      for (const asset of activeAssets) {
        ...
        if (depInRange > 0) {
          ...
          results.push({
            ...
            categoryAccountCode: cat?.accountCode || asset.categoryAccountCode,
            categoryName: catName,
            depExpCode: cat?.depExpCode || null,
            accumCode: cat?.accumCode || null,
