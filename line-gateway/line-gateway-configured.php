<?php
/**
 * LINE Gateway — Transferable PHP Proxy (v2.1 with SQLite Queue)
 * 
 * Drop this single file on any PHP 8.2+ hosting with HTTPS.
 * No MySQL needed — uses SQLite (built-in to PHP, zero setup).
 * Configure the settings below, then point LINE webhook URL here.
 * 
 * Handles:
 *   POST /line-gateway-configured.php              → forwards LINE webhook to APP_SERVER (queue on failure)
 *   POST /line-gateway-configured.php?action=push  → forwards push request to LINE API
 *   POST /line-gateway-configured.php?action=reply → forwards reply message to LINE API
 *   POST /line-gateway-configured.php?action=api   → proxies any LINE API call
 *   GET  /line-gateway-configured.php?action=ping  → health check
 *   GET  /line-gateway-configured.php?action=info  → gateway status + config (no secrets)
 *   GET  /line-gateway-configured.php?action=stats → daily usage stats (3-month history)
 *   POST /line-gateway-configured.php?action=drain → app server pulls pending webhooks
 *   POST /line-gateway-configured.php?action=ack   → app server confirms processed webhooks
 *   GET  /line-gateway-configured.php?action=queue → view queue status
 */
...
// ============================================================
$CONFIG = [
    'LINE_CHANNEL_ACCESS_TOKEN' => 'HCjHIjkBloqZJhB+uJFf8ybkCojtqqIJXELqaOHTMujTCxoXJX1CDxb6zBQOXXSQgvq9NT74gRHUG2IFgVzsxYQ/QEboP3RJIbl5o/JzieXfPHZE33Cbr/2S5IVEDPBdMrnlwcSFMm3HypCYTsp+AAdB04t89/1O/w1cDnyilFU=',
    'LINE_CHANNEL_SECRET'       => '04e7548d2f0c3ac497c607ed951805a6',
    'APP_SERVER_WEBHOOK_URL'    => 'https://etaxerp.com/api/line/webhook',

    'AUTH_KEY'                  => '4dfea863376e9eb06a64b7bba532058331d6ad76aaf7835e',
    'LOG_ENABLED'              => true,
    ...
    'SQLITE_FILE'              => __DIR__ . '/line-gateway.db',
...
function get_db() {
    ...
    try {
        ...
        gateway_log("SQLITE ERROR — " . $e->getMessage());
        return null;
    }
}
...
function ensure_queue_table($pdo) {
    ...
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_status_retry ON webhook_queue (status, next_retry_at)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_created ON webhook_queue (created_at)");
}

function queue_webhook($body, $signature, $sourceIp, $error, $httpCode) {
    $db = get_db();
    if (!$db) {
        gateway_log("QUEUE FAILED — SQLite unavailable, webhook LOST");
        return false;
    }
    try {
        $stmt = $db->prepare("INSERT INTO webhook_queue (payload, signature, source_ip, last_error, last_http_code, next_retry_at) VALUES (?, ?, ?, ?, ?, datetime('now', '+1 minute'))");
...
if ($method === 'GET' && $action === 'info') {
    ...
    json_response([
        'gateway'      => 'LINE Gateway v2.1 (SQLite, forward-first)',
...
if ($method === 'POST' && $action === 'drain') {
    ...
    $db = get_db();
    if (!$db) json_response(['error' => 'SQLite unavailable'], 500);
...
if ($method === 'POST' && $action === 'ack') {
    ...
    if (!empty($delivered)) {
        ...
        $db->prepare("UPDATE webhook_queue SET status='delivered', delivered_at=datetime('now') WHERE id IN ({$placeholders})")->execute($delivered);
    ...
    if (!empty($failed)) {
        foreach ($failed as $item) {
            ...
            $db->prepare("UPDATE webhook_queue SET status='pending', retries=retries+1, last_error=?, next_retry_at=datetime('now', '+' || MIN(CAST(POWER(2, retries) AS INTEGER), 60) || ' minutes') WHERE id=?")->execute([$error, $id]);
        }
    ...
    $db->exec("DELETE FROM webhook_queue WHERE status='delivered' AND delivered_at < datetime('now', '-{$retentionDays} days')");
    $db->exec("DELETE FROM webhook_queue WHERE status='failed' AND created_at < datetime('now', '-{$retentionDays} days')");
...
// --- Forward LINE webhook to app server (queue only on failure) ---
...
json_response([
    ...
    'usage' => [
        'webhook'  => 'POST /line-gateway-configured.php (LINE sends here)',
        'push'     => 'POST /line-gateway-configured.php?action=push (app sends push messages)',
        'reply'    => 'POST /line-gateway-configured.php?action=reply (app sends reply messages)',
        'api'      => 'POST /line-gateway-configured.php?action=api (app proxies LINE API calls)',
        'ping'     => 'GET  /line-gateway-configured.php?action=ping (health check)',
        'info'     => 'GET  /line-gateway-configured.php?action=info (gateway status)',
        'stats'    => 'GET  /line-gateway-configured.php?action=stats&auth=KEY (usage stats)',
    ],
], 400);
